...
const Dashboard: React.FC<{initialProfile: UserProfile}> = ({ initialProfile }) => {
    ...
    const tabs: { id: Tab; label: string; icon: React.ReactNode }[] = [
        ...
        { id: 'plan', label: t.tab_plan, icon: <CalendarIcon className="w-5 h-5" /> },
    ];
    ...
    return (
        <div className="flex flex-col min-h-screen">
            <header className="bg-black/30 backdrop-blur-sm sticky top-0 z-10 border-b border-gray-700/50">
                        ...
                        <div className="flex items-center gap-2">
                         ...
                         <div className="flex items-center gap-4">
                            <div className={language === 'ar' ? 'text-left' : 'text-right'}>
                               <p className="text-sm font-medium text-white">{t.welcome_user.replace('{name}', userProfile?.name.split(' ')[0] || '')}</p>
                               <p className="text-xs text-slate-400">{t.journey_starts}</p>
                            </div>
                            <LanguageSwitcher />
                        </div>
                    </div>
